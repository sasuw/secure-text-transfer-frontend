    ...
    getStringForPin(pinInput.value).then(response => {
        try {
            if (response.status === 204) {
                ...
                pwdValueSpan.innerText= 'Kein Text mit der PIN gefunden';
                hide('stringStart');
                hide('showText');
            } else {
                response.text().then(data => {
...
function pwdTimeExpired() {
    document.getElementById('stringStart').innerText = 'Dein Text ist abgelaufen';
    document.getElementById('pwdValue').innerText = '';
    hide('pwdHint');
    document.getElementById('gotitPwd').value = 'Alles klar';
}
...
function pinTimeExpired() {
    ...
    document.getElementById('gotitPin').value = 'Alles klar';
}
